	...
	def prepareMainTemplate(self):
		...
		if ret_brand == "technomate":
			if open("/proc/stb/info/hwmodel","r").read().strip().lower() in ("tmnanooe","tmsingle"):
				...
				if open("/proc/stb/info/hwmodel","r").read().strip().lower() in ("force1plus", "force2plus"):
					ret["remote"] = "te_type3"
					...
					ret["remote"] = "te_type2"
				elif open("/proc/stb/info/hwmodel","r").read().strip().lower() in ("tmnanose","tmnanosecombo"): # change tmnanose model rcu
					ret["remote"] = "te_type3"
				elif open("/proc/stb/info/hwmodel","r").read().strip().lower() in ("tmnanosem2","tmnanoseplus"): # tmnanosem2 model rcu
					ret["remote"] = "te_type4"
				elif open("/proc/stb/info/hwmodel","r").read().strip().lower() in ("tmnanom3"): # tmnanom3 model rcu
					ret["remote"] = "tmnanom3"
				else:
					ret["remote"] = "te_type0"
